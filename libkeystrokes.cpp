...
static void drawmenu() {
    ...
    // Long Press Detection (0.8s)
    if (ImGui::IsWindowHovered() && ImGui::IsMouseReleased(0)) {
        if (ImGui::GetIO().MouseDownDuration[0] > 0.8f) {
            g_show_config = !g_show_config;
        }
    }

    // Move Logic
    if (!g_is_locked && ImGui::IsWindowHovered() && ImGui::IsMouseDragging(0)) {
    ...
    // v1.0.1 Configuration Menu (Minecraft Style)
    if (g_show_config) {
        ...
        ImGui::Text("Size: %.0fpx", 60.0f * g_ui_scale);
        ...
        if (ImGui::Button("Done", ImVec2(-1, 0))) g_show_config = false;
        ImGui::End();
    }
}
