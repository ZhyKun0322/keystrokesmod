...
// EXACT address from your screenshot - 0xe640f48
#define MOVERELATIVE_ADDR 0xe640f48
...
// InputConsumer::consume variants
...
// moveRelative function signature
...
struct CpsTracker {
    ...
    void click() {
        ...
        times[head] = std::chrono::duration<double>(now).count();
...
static void hook_moveRelative(void* self, float strafe, float forward, float vertical, float speed) {
...

    {
        ...
        // Detect movement direction from touch D-pad
        // Only update if values are reasonable (prevent garbage values)
        if (speed >= 0.0f && speed <= 1.0f) {
            // Forward/backward
            if (forward > 0.01f) g_keys.w = true;
            else if (forward < -0.01f) g_keys.s = true;
            else {
                // Only clear if not pressed by keyboard
                // We keep the key state true if either keyboard OR touch is pressing
            }
            
            // Left/right
            if (strafe < -0.01f) g_keys.a = true;
            else if (strafe > 0.01f) g_keys.d = true;
        }
    }

    // Call original function
    if (orig_moveRelative) {
        orig_moveRelative(self, strafe, forward, vertical, speed);
    }
}
...
static void drawkeycps(const char* label, bool pressed, ImVec2 size, int cps) {
    ...
    dl->AddText(fn, fs, ImVec2(pos.x + (size.x - labelSz.x) * 0.5f, blockTop),
                ImGui::ColorConvertFloat4ToU32(textcolor), label);

    ImVec4 dimcol = ImVec4(textcolor.x, textcolor.y, textcolor.z, textcolor.w * 0.70f);
    dl->AddText(fn, smallfs, ImVec2(pos.x + (size.x - cpsSz.x) * 0.5f, blockTop + labelSz.y + gap),
...
static void drawsettings(ImVec2 hudpos) {
    ...
    if (g_consume_variant >= 0)
        ImGui::TextColored(ImVec4(0.40f, 0.80f, 0.40f, 1.0f), "KB Hook: OK (variant %d)", g_consume_variant);
    else
...
static void setup() {
    ...
    g_initialized = true;
    LOGI("ImGui initialized: %dx%d scale=%.2f", g_width, g_height, dpscale);
}
...
static EGLBoolean hook_eglswapbuffers(EGLDisplay dpy, EGLSurface surf) {
    ...
    if (ctx == EGL_NO_CONTEXT) {
        return orig_eglswapbuffers ? orig_eglswapbuffers(dpy, surf) : EGL_FALSE;
    }
    ...
    if (g_targetcontext == EGL_NO_CONTEXT) {
        ...
        LOGI("Target context acquired: %p", (void*)ctx);
    }
...
static void getmcbasesize(uintptr_t& base, size_t& size) {
    ...
    if (first_start && last_end > first_start) {
        ...
        LOGI("libminecraftpe.so: base=%p, size=%zu", (void*)base, size);
    }
}

static void* pattern_scan(uintptr_t base, size_t size, const uint8_t* pattern, size_t patlen) {
...
static void* mainthread(void*) {
    ...
    if (swap) {
        if (GlossHook(swap, (void*)hook_eglswapbuffers, (void**)&orig_eglswapbuffers)) {
    ...
    if (mc_base && mc_size) {
        uintptr_t target_addr = mc_base + MOVERELATIVE_ADDR;
        
        LOGI("Attempting to hook moveRelative at %p (base=%p + offset=0x%x)", 
             (void*)target_addr, (void*)mc_base, MOVERELATIVE_ADDR);
        ...
        static const uint8_t expected_pattern[] = {
            ...
            0xFD, 0x03, 0x00, 0x91, 0xFF, 0x03, 0x3E, 0xD1
        };
        
        if (memcmp((void*)target_addr, expected_pattern, sizeof(expected_pattern)) == 0) {
            LOGI("Pattern verified at target address!");
            
            if (GlossHook((void*)target_addr, (void*)hook_moveRelative, (void**)&orig_moveRelative)) {
                g_moverelative_hooked = true;
                LOGI("moveRelative hooked successfully at %p", (void*)target_addr);
            } else {
                LOGE("GlossHook failed for moveRelative");
            }
        } else {
            LOGW("Pattern mismatch at target address! Scanning for pattern...");
            
            void* found = pattern_scan(mc_base, mc_size, expected_pattern, sizeof(expected_pattern));
            if (found) {
                LOGI("Found pattern at %p (offset: 0x%lx)", found, (uintptr_t)found - mc_base);
                
                if (GlossHook(found, (void*)hook_moveRelative, (void**)&orig_moveRelative)) {
                    g_moverelative_hooked = true;
                    LOGI("moveRelative hooked successfully at found address");
                } else {
                    LOGE("GlossHook failed for found address");
                }
            } else {
                LOGW("Pattern not found in libminecraftpe.so");
            }
        }
    } else {
        LOGW("Could not get libminecraftpe.so base address");
    }

    LOGI("Initialization complete. KB: %s, Touch: %s",
         g_consume_variant >= 0 ? "OK" : "FAIL",
         g_moverelative_hooked ? "OK" : "FAIL");
