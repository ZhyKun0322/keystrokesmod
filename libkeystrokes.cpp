...
static void drawmenu() {
    ...
    if (ImGui::IsWindowHovered()) {
        // Long Press to open Settings (0.8 seconds)
        if (ImGui::IsMouseReleased(0) && ImGui::GetIO().MouseDownDuration[0] > 0.8f) {
            g_show_config = !g_show_config;
        }

        // Draggable Logic (if not locked)
        if (!g_is_locked && ImGui::IsMouseDragging(0)) {
    ...
    // --- Configuration Menu (Minecraft Style) ---
    if (g_show_config) {
        ImGui::SetNextWindowSize(ImVec2(320, 0));
        ImGui::SetNextWindowPos(ImVec2(g_width/2, g_height/2), ImGuiCond_Always, ImVec2(0.5f, 0.5f));
        ImGui::Begin("HUD Settings", &g_show_config, ImGuiWindowFlags_NoResize | ImGuiWindowFlags_AlwaysAutoResize);
        
        ImGui::Text("Size: %.0fpx", keysize);
        ImGui::SliderFloat("##size", &g_ui_scale, 0.5f, 2.5f, "");
        
        ImGui::Text("Opacity: %.0f%%", g_opacity * 100.0f);
        ImGui::SliderFloat("##alpha", &g_opacity, 0.1f, 1.0f, "");
        
        ImGui::Separator();
        ImGui::Checkbox("Lock Position", &g_is_locked);
        ImGui::TextDisabled("Prevents accidental dragging during play");

        if (ImGui::Button("Done", ImVec2(-1, 40))) g_show_config = false;
...
static EGLBoolean hook_eglswapbuffers(EGLDisplay dpy, EGLSurface surf) {
    ...
    if (g_targetcontext == EGL_NO_CONTEXT) {
        ...
        LOGD("Target EGL Context captured: %p", ctx);
    }
...
static void* mainthread(void*) {
    // Increased sleep to ensure game libraries are fully mapped
    sleep(6);
    GlossInit(true);
    ...
    // Using the exact symbol found via nm in Termux
    const char* input_sym = "_ZN7android13InputConsumer7consumeEPNS_26InputEventFactoryInterfaceEblPjPPNS_10InputEventE";
    void* symconsume = (void*)GlossSymbol(hlib, input_sym, nullptr);
    
    if (symconsume) {
        GlossHook(symconsume, (void*)hook_consume, (void**)&orig_consume);
        LOGD("Input hook applied to: %s", input_sym);
    } else {
        LOGD("Critical Error: Could not find input symbol!");
    }
